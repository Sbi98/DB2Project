...
begin
    ...
    set nOfAnswers = (select count(*) from DB2Project.manswer A where A.review = old.id); 
    set curr_points = (select points from DB2Project.user U where U.id = old.user); 
	update DB2Project.user U set points = curr_points - tmp_points - nOfAnswers where U.id = old.user;
end; $$
DELIMITER ;
